...
public abstract class MinMaxNode {
	...
	public int nbHole = Board.NB_HOLES;
	...
	public MinMaxNode(Board board, int depth, double alpha, double beta) {
		 ...
		 * possible
		 */
		this.decision = new double[nbHole];
		...
		for (int i = 0; i < nbHole; i++) {
			...
			if (board.getPlayerHoles()[i] != 0 && !this.alphabeta(this.evaluation, alpha, beta)) {

				/* Sélection du coup à jouer */
				double[] decision = new double[nbHole];
				decision[i] = 1;
				...
				try {
					...
					if ((score < 0) || (copy.getScore(Board.otherPlayer(copy.getCurrentPlayer())) >= 25)
							|| (copy.getNbSeeds() <= 6))
						this.decision[i] = copy.getScore(MinMaxNode.player)
								- copy.getScore(Board.otherPlayer(MinMaxNode.player));
					/* Sinon, on explore les coups suivants */
						...
						if (depth < MinMaxNode.maxDepth) {
							...
							this.decision[i] = child.evaluation;
						}
						/*
	...
	/**
	 * Retourne le min ou la max entre deux valeurs, selon le type de noeud (MinNode
	 * ou MaxNode)
	 * 
	...
	/**
	 * Indique s'il faut faire une coupe alpha-beta, selon le type de noeud (MinNode
	 * ou MaxNode)
	 * 
