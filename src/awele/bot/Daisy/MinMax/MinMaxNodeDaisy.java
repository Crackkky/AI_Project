package awele.bot.Daisy.MinMax;
...
public abstract class MinMaxNodeDaisy {
    ...
    /**
     * Profondeur intermediaire
     */
    private static int depthLimit = 3;

    /**
     * Tableau de score pour chaque noeud
     */
    private int[] scores;
    ...
    public MinMaxNodeDaisy(Board board, int depth, double alpha, double beta) {
        ...
        /*Initialisation des scores pour ce noeuds*/
        this.scores = new int[Board.NB_HOLES];
            ...
            if (board.getPlayerHoles()[i] != 0) {
                ...
                try {
                    ...
                    if ((score < 0) ||
                        ...
                        /*Si la profondeur intermédiaire n'est pas atteinte*/
                        if(depth >= MinMaxNodeDaisy.depthLimit) {
                            MinMaxNodeDaisy.depthLimit += depth;
        ...
        String[] typeNode = this.getClass().getName().split("[.]");
        System.out.println("Node type : " + typeNode[typeNode.length -1]);
        for (int i = 0; i < Board.NB_HOLES; i++)
     ...
     * Initialisation
     */
    public static void initialize(Board board, int maxDepth, int depthLimit) {
        MinMaxNodeDaisy.maxDepth = maxDepth;
        MinMaxNodeDaisy.depthLimit = depthLimit;
